...
MojErr MojDbSandwichSeq::open(const MojChar* name, MojDbSandwichDatabase* db)
{
    ...
    bool found = false;
    err = m_db->get(MojDbIdGenerator::MainShardId, m_key, NULL, false, val, found);
    MojErrCheck(err);
...
MojErr MojDbSandwichSeq::close(MojDbStorageTxn* txn)
#else
...
#ifdef LMDB_ENGINE_SUPPORT
MojErr MojDbSandwichSeq::get(MojInt64& valOut, MojDbStorageTxn* txn)
#else
...
MojErr MojDbSandwichSeq::store(MojUInt64 next)
{
    ...
    MojErrCheck(err);
    err = m_db->put(MojDbIdGenerator::MainShardId, m_key, val, NULL, false);
    MojErrCheck(err);
