...
MojErr MojDbCursor::visit(MojObjectVisitor& visitor)
{
    ...
    if(m_aggregateFilter.get() != NULL) {
        MojErr err = m_aggregateFilter->visit(visitor);
        MojErrCheck(err);
    }
...
MojErr MojDbCursor::count(MojUInt32& countOut)
{
    ...
    MojErrThrow(MojErrNotOpen);
    if(m_aggregateFilter.get() != NULL) {
        countOut = static_cast<MojUInt32>(m_aggregateFilter->count());
        return MojErrNone;
    }
...
MojErr MojDbCursor::initImpl(const MojDbQuery& query)
{
    ...
    if (!query.aggregate().empty()) {
        m_aggregateFilter.reset(new MojDbAggregateFilter);
        MojAllocCheck(m_aggregateFilter.get());
        MojErr err = m_aggregateFilter->init(query);
        MojErrCheck(err);
    }
	m_query = query;
...
MojErr MojDbCursor::visitObject(MojObjectVisitor& visitor, bool& foundOut)
{
    ...
    if (foundOut) {
        if(m_aggregateFilter.get() != NULL) {
            ...
            MojErrCheck(err);
            err = m_aggregateFilter->aggregate(obj);
            MojErrCheck(err);
        } else {
...
#ifdef LMDB_ENGINE_SUPPORT
void MojDbCursor::txn(MojRefCountedPtr<MojDbStorageTxn> txn, bool ownTxn)
{
   LOG_TRACE("Entering function %s", __FUNCTION__);

	MojAssert(!m_txn.get() && txn.get());
	m_txn = txn;
	m_ownTxn = ownTxn;
}
#else
