...
: m_spaceAlert(*this),
  ...
  m_isOpen(false),
  m_enableRootKind(true),
#ifdef LMDB_ENGINE_SUPPORT
  m_enablePurge(true),
  m_isDbAppLocking(true)
#else
  m_enablePurge(true)
#endif
{
...
MojErr MojDb::configure(const MojObject& conf)
{
	...
	if (conf.get(ConfKey, dbConf)) {
        ...
        found = dbConf.get(_T("enableRootKind"), m_enableRootKind);
        if (!found) {
            m_enableRootKind = true;
        }
        if (m_enableRootKind) {
            // only if m_enableRootKind is true ('Object:1' kind is exist)
            // purge can be executable
            found = dbConf.get(_T("enablePurge"), m_enablePurge);
            if (!found) {
                // default is true
                m_enablePurge = true;
            }
		...
		err = m_profileEngine.configure(dbConf);
		MojErrCheck(err);
#ifdef LMDB_ENGINE_SUPPORT
		MojObject engineConf;
		found = conf.get(m_engineName, engineConf);
		if (found) {
			found = engineConf.get(_T("noLock"), m_isDbAppLocking);
		} else {
			found = conf.get(m_storageEngine.get()->engineFactory()->name(), engineConf);
			if (found) {
				engineConf.get(_T("noLock"), m_isDbAppLocking);
			}
		}
#endif
	}
...
MojErr MojDb::open(const MojChar* path, MojDbStorageEngine* engine)
{
	...
	err = m_storageEngine->openExtDatabase(ObjDbName, req.txn(), m_objDb);
	MojErrCheck(err);
	...
	err = m_profileEngine.open(this, req);
	MojErrCheck(err);
...
MojErr MojDb::delKind(const MojObject& id, bool& foundOut, MojUInt32 flags, MojDbReqRef req)
{
	...
	err = kindEngine()->getKind(idStr.data(), pk);
	MojErrCheck(err);
	...
	if (item.get())
	{
		...
		MojUInt32 count;
		req->fixmode(true);
		err = delImpl(query, count, req, flags | MojDbFlagPurge);
		MojErrCheck(err);
...
MojErr MojDb::recursiveDelKind(const MojObject& id, bool& foundOut, MojUInt32 flags, MojDbReqRef req)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
    ...
    if (pk->nsubkinds() > 0)
    {
        pk->getOwnSubKinds(subKinds);

        for (MojVector<MojDbKind*>::ConstIterator i = subKinds.begin(); i != subKinds.end(); ++i)
        {
            MojObject idk((*i)->id());
            err = recursiveDelKind(idk, foundOut, flags, req);
            MojErrCheck(err);
        }
    }

    err = delKind(id, foundOut, flags, req);
    MojErrCheck(err);
...
MojErr MojDb::put(MojObject* begin, const MojObject* end, MojUInt32 flags, MojDbReqRef req, MojString shardId)
{
	...
	for (MojObject* i = begin; i != end; ++i) {
        ...
        MojErrCheck(err);
        err = shardEngine()->linkShardAndKindId(shardId, kindId, req);
        MojErrCheck(err);
	}
	err = req->end();
	MojErrCheck(err);
...
MojErr MojDb::putKind(MojObject& obj, MojUInt32 flags, MojDbReqRef req)
{
    ...
    if (false == isSame)
    {
        ...
        err = putImpl(obj, flags | MojDbFlagForce, req, true, MojString(), false);
        MojErrCheck(err);
        guard.unset();
...
MojErr MojDb::watch(const MojDbQuery& query, MojDbCursor& cursor, WatchSignal::SlotRef watchHandler, bool& firedOut, MojDbReqRef req)
{
	...
	shardStatusChanged.connect(watcher->shardStatusChangedSlot);
...
MojErr MojDb::putObj(const MojObject& id, MojObject& obj, const MojObject* oldObj,
                     ...
                     MojString shardIdStr, bool reverseTransaction)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
	...
	MojObject putId = id;
	if (putId.undefined()) {
		err = m_idGenerator.id(putId, shardIdStr);
		MojErrCheck(err);
    ...
    MojDbKind *kind = NULL;
    err = m_kindEngine.getKind(obj, kind);
    MojErrCheck(err);
#ifdef LMDB_ENGINE_SUPPORT
	kind->setTxn(req.txn());
#endif

    if (!kindName.startsWith(MojDbKindEngine::KindKindIdPrefix) && kind->assignId()) {
#ifdef LMDB_ENGINE_SUPPORT
		err = assignIds(obj, req.txn());
#else
		err = assignIds(obj);
#endif
		MojErrCheck(err);
	...
	if (shardIdStr.empty()) {
		...
		MojObjectWriter writer(buf, &tokenSet, false);
		err = obj.visit(writer);
	}
	MojErrCheck(err);

	// retrive effective shard id
	MojDbShardId shardId;
	err = MojDbIdGenerator::extractShard(putId, shardId);
	MojErrCheck(err);

	// store it in the db
	if (oldItem) {
		err = m_objDb->update(shardId, putId, buf, oldItem, req.txn());
		MojErrCheck(err);
	} else {
		err = m_objDb->insert(shardId, putId, buf, req.txn());
		MojErrCheck(err);
	}
...
MojErr MojDb::delObj(const MojObject& id, const MojObject& obj, MojDbStorageItem* item, MojObject& foundObjOut, MojDbReq& req, MojUInt32 flags)
{
    ...
    if (!m_enablePurge) {
        // If m_enablePurge flag is 'false'
        // Force to purge
        flags |= MojDbFlagPurge;
    }
	if (MojFlagGet(flags, MojDbFlagPurge)) {
		// update indexes
		MojTokenSet tokenSet;
		// we want purge to force delete
		req.fixmode(true);
		err = m_kindEngine.update(NULL, &obj, req, OpDelete, tokenSet);
		MojErrCheck(err);
		// permanently delete
		bool found = false;
		err = m_objDb->del(shardId, id, req.txn(), found);
		MojErrCheck(err);
		if (!found)
		...
		err = newObj.putBool(DelKey, true);
		MojErrCheck(err);
...
MojErr MojDb::delImpl(const MojObject& id, bool& foundOut, MojObject& foundObjOut, MojDbReq& req, MojUInt32 flags)
{
	...
	MojRefCountedPtr<MojDbStorageItem> item;
	err = m_objDb->get(shardId, id, req.txn(), true, item);
	MojErrCheck(err);
	if (item.get()) {
		// and delete it
		MojObject obj;
...
MojErr MojDb::getImpl(const MojObject& id, MojObjectVisitor& visitor, MojDbOp op, MojDbReq& req)
{
	...
	MojRefCountedPtr<MojDbStorageItem> item;
	err = m_objDb->get(shardId, id, req.txn(), false, item);
	MojErrCheck(err);
	if (item.get()) {
		MojString kindId;
...
/**
 * TODO: Make more accurate check of kinds
 */
...
MojErr MojDb::putImpl(MojObject& obj, MojUInt32 flags, MojDbReq& req, bool checkSchema, MojString shardIdStr, bool reverseTransaction)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
	...
	if (obj.get(IdKey, id)) {
		...
		// get previous revision if we have an id
		err = m_objDb->get(shardId, id, req.txn(), true, prevItem);
		MojErrCheck(err);
	}
	if (MojFlagGet(flags, MojDbFlagIgnoreMissing) && MojFlagGet(flags, MojDbFlagMerge)) {
		if (!prevItem.get()) {
	...
	if (prevItem.get()) {
		...
		if (MojFlagGet(flags, MojDbFlagMerge)) {
			// do merge
			MojObject merged;
			err = mergeInto(merged, obj, prev);
			MojErrCheck(err);
			obj = merged;
		} else if (!MojFlagGet(flags, MojDbFlagForce)) {
	...
	if (prevPtr != NULL && obj.get(RevKey, newRev)) {
		...
		MojErrCheck(err);
		if (!MojFlagGet(flags, MojDbFlagForce) && newRev != oldRev)
			MojErrThrowMsg(MojErrDbRevisionMismatch, _T("db: revision mismatch - expected %lld, got %lld"), oldRev, newRev);
	}

	// save it
	MojErr err = putObj(id, obj, prevPtr, prevItem.get(), req, op, checkSchema, shardIdStr, reverseTransaction);
	MojErrCheck(err);
...
#ifdef LMDB_ENGINE_SUPPORT
MojErr MojDb::nextId(MojInt64& idOut, MojDbStorageTxn* txn)
#else
...
#endif
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
#ifdef LMDB_ENGINE_SUPPORT
	MojErr err = m_idSeq->get(idOut, txn);
#else
...
MojErr MojDb::getLocale(MojString& valOut, MojDbReq& req)
{
	...
	MojErr err = beginReq(req);
	MojErrCheck(err);
	err = getState(LocaleKey, curLocale, isFound, req);
	MojErrCheck(err);
	err = req.end(false);
#else
	MojErr err = getState(LocaleKey, curLocale, isFound, req);
#endif
	MojErrCheck(err);
	if (isFound) {
...
MojErr MojDb::getState(const MojChar* key, MojObject& valOut, bool& found, MojDbReq& req)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
...
MojErr MojDb::updateState(const MojChar* key, const MojObject& val, MojDbReq& req)
{
	...
	err = obj.put(key, val);
	MojErrCheck(err);
	err = putImpl(obj, MojDbFlagMerge, req);
	MojErrCheck(err);
...
#ifdef LMDB_ENGINE_SUPPORT
MojErr MojDb::assignIds(MojObject& obj, MojDbStorageTxn* txn)
#else
...
MojErr MojDb::removePrivateDataByOwner(const MojString& owner, MojDbReqRef req)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
...
MojErr MojDb::initDatabaseId(MojDbReqRef req)
{
    MojObject id;
    MojErr err;
    ...
    bool isFound;
    err = getState(_T("databaseId"), id, isFound, req);
    MojErrCheck(err);

    if (!isFound) {
        err = m_idGenerator.id(id);
        MojErrCheck(err);
        err = updateState(_T("databaseId"), id, req);
        MojErrCheck(err);
    }
    ...
    err = id.stringValue(m_databaseId);
    MojErrCheck(err);
    MojAssert(!m_databaseId.empty());

    return MojErrNone;
}

MojErr MojDb::databaseId(MojString& id, MojDbReqRef req)
{
    MojErr err;
    if (m_databaseId.empty()) {
        err = initDatabaseId(req);
        MojErrCheck(err);
    }
...
MojErr MojDb::profile(const MojChar* application, bool enable, MojDbReqRef req)
{
	MojErr err;

	if (!m_profileEngine.isEnabled()) {
		MojErrThrow(MojErrDbProfileDisabled);
	}

	err = beginReq(req, true);
	MojErrCheck(err);

	err = m_profileEngine.profile(application, enable, req);
	MojErrCheck(err);

    return MojErrNone;
}

MojErr MojDb::releaseAdminProfile(const MojChar* application, MojDbReqRef req)
{
	MojErr err;
	...
	err = beginReq(req, true);
	MojErrCheck(err);
	err = m_profileEngine.releaseAdminProfile(application, req);
	MojErrCheck(err);

	return MojErrNone;
}

MojErr MojDb::profileStats(const MojChar* application, MojObjectVisitor* writer, MojDbReqRef req, MojObject& query)
{
	MojErr err;
	...
	err = m_profileEngine.getStats(application, writer, req, query);
	MojErrCheck(err);
