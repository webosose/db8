...
MojErr MojService::CategoryHandler::addMethod(const MojChar* methodName, Callback callback, const MojChar* schemaJson, uint32_t ls2_method_flags)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
	...
	err = m_callbackMap.put(str, CallbackInfo(callback, schema.get(), ls2_method_flags));
	MojErrCheck(err);

	return MojErrNone;
}

MojErr MojService::CategoryHandler::addMethods(const Method* methods)
{
    LOG_TRACE("Entering function %s", __FUNCTION__);
	MojAssert(methods);

	for (const Method* method = methods; method->m_name != NULL; method++) {
		MojErr err = addMethod(method->m_name, method->m_callback, NULL, method->m_ls2_method_flags);
		MojErrCheck(err);
	}
	return MojErrNone;
}

MojErr MojService::CategoryHandler::addMethods(const SchemaMethod* methods)
{
	LOG_TRACE("Entering function %s", __FUNCTION__);
	MojAssert(methods);

	for (const SchemaMethod* method = methods; method->m_name != NULL; method++) {
		MojErr err = addMethod(method->m_name, method->m_callback, method->m_schemaJson);
		MojErrCheck(err);
	}
	return MojErrNone;
}
...
MojErr MojService::CategoryHandler::addMethods(const Method* methods, bool)
{
    return addMethods(methods);
}

MojErr MojService::CategoryHandler::addMethods(const SchemaMethod* methods, bool)
{
    return addMethods(methods);
}
